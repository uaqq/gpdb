		...
		RelationIsAppendOptimized(onerel))
	{
		...
		if (vacstmt->appendonly_phase == AOVAC_COMPACT)
		{
				...
				linitial_int(vacstmt->appendonly_compaction_insert_segno) == APPENDONLY_COMPACTION_SEGNO_INVALID)
			{
				...
				UpdateMasterAosegTotalsFromSegments(onerel, appendOnlyMetaDataSnapshot, vacstmt->appendonly_compaction_insert_segno, 0, false);

			UpdateMasterAosegTotalsFromSegments(onerel, appendOnlyMetaDataSnapshot, vacstmt->appendonly_compaction_segno, 0, true);
		}
		else if (vacstmt->appendonly_phase == AOVAC_DROP)
		{
			...
			UpdateMasterAosegTotalsFromSegments(onerel, appendOnlyMetaDataSnapshot, vacstmt->appendonly_compaction_segno, 0, false);
		}
