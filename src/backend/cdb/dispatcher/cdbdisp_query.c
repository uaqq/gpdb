...
CdbDispatchSetCommand(const char *strCommand, bool cancelOnError)
{
	...
	queryText = buildGpQueryString(pQueryParms, &queryTextLength, false);
                                ...
                                CdbPgResults *cdb_pgresults)
{
	...
	char *queryText;
	int queryTextLength;
	bool isAnalyze = flags & DF_IS_ANALYZE;
	...
	queryText = buildGpQueryString(pQueryParms, &queryTextLength, isAnalyze);
