                           ...
                           CdbPathLocus locus)
{
    ...
    else if (CdbPathLocus_IsReplicated(subpath->locus))
    {
        ...
        /*
         * Assume that the return of modifying CTE can be redistributed to
    ...
    else if (CdbPathLocus_IsSegmentGeneral(subpath->locus))
    {
        ...
        {
            /*
             * Motion is allowed in two cases. Firstly, when replicated table
             * is updated or deleted by join with other replicated table,
             * whose numsegments is less than the former one's. Secondly, when
             * join between CTE with DML operations over a replicated table
             * and other replicated table takes place. Second case takes place
             * when SegmentGeneral's numsegments is less than Replicated's.
             */
                        ...
                        bool inner_require_existing_order)
{
     ...
     * correct update/delete. Another case - DML over replicated table inside
     * CTE with SELECT above.
     */
             ...
             CdbPathLocus_IsReplicated(inner.locus))
    {
        ...
        else if (hasModifyingCTESelectRecur(root))
        {
            ...
            else if (CdbPathLocus_IsPartitioned(other->locus))
            {
                ...
                else if (CdbPathLocus_NumSegments(replicated->locus) >=
                         CdbPathLocus_NumSegments(other->locus))
                    return other->locus;

                /*
                 * For the case that other is a Hashed table and redistribute
                 * clause matches other's distribute keys and , we could
                 * redistribute Replicated to other.
                 */
                else if (CdbPathLocus_IsHashed(other->locus) &&
                         cdbpath_match_preds_to_distkey(root,
                                                        redistribution_clauses,
                                                        other->path,
                                                        other->locus,
                                                        &replicated->move_to))
                    /*
                     * the result is distributed on the same segments with the
                     * other
                     */
                    AssertEquivalent(CdbPathLocus_NumSegments(other->locus),
                              CdbPathLocus_NumSegments(replicated->move_to));
                /* Otherwise gather both of them to a SingleQE */
                else
                {
