...
shareinput_walker2(Node *node, ShareInputContext *context)
{
	...
	if (IsA(node, SubPlan))
	{
		...
		forboth(lp, root->glob->subplans, lr, root->glob->subroots)
		{
			...
			ret = shareinput_walker2((Node *) i_subplan, &context2);
	...
	else if (recursive_down)
	{
		 ...
		 if (IsA(node, SubqueryScan))
		{
			...
			context2.root = subroot;
			ret = shareinput_walker2((Node *) subqscan->subplan, &context2);
		...
		else if (IsA(node, TableFunctionScan))
		{
			...
			context2.root = subroot;
			ret = shareinput_walker2((Node *)  tfscan->scan.plan.lefttree, &context2);
