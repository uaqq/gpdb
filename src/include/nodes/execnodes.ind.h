...
typedef struct ResultRelInfo
{
	...
	/*
	 * Extra GPDB junk columns. ri_segid_attno is used with DELETE, to
	 * indicate the segment the target tuple came from. 'action' is used with
	 * Split Updates.
	 *
	 * The target tuple's ctid is in ri_junkFilter->jf_junkAttNo, like in
	 * upstream.
	 */
...
typedef struct EState
{
	...
	bool		cancelUnfinished;	/* when we're cleaning up, we need to make
									 * sure that we know it */
	...
	/*
	 * The slice number for the current node that is being processed. During
	 * plan initialization, in ExecInitPlan(), it is set to the slice we're
	 * currently initializing, even if it's an "alien" node. When executing a
	 * plan (ExecProcNode()), it is always set to the local slice we're
	 * currently executing, never to an alien slice.
	 */
	int			currentSliceId;
	...
	/*
	 * partition oid that is being scanned, used by
	 * DynamicBitmapHeapScan/IndexScan
	 */
	int			partitionOid;
...
typedef struct MemoryManagerContainer
{
	...
	/*
	 * If existing space is too small, the realloced space is how many times
	 * of the existing one.
	 */
	int			realloc_ratio;
...
typedef struct ModifyTableState
{
	...
	bool	   *mt_isSplitUpdates;	/* per-subplan flag to indicate if it's a
									 * split update */
...
typedef struct DynamicIndexScanState
{
	...
	/*
	 * IndexScanState and IndexOnlyScanState are mutually exclusive fields
	 * used set for dynamic index scan or dynamic index only scan.
	 */
	...
	/*
	 * This memory context will be reset per-partition to free up previous
	 * partition's memory
	 */
	...
	struct PartitionPruneState *as_prune_state; /* partition dynamic pruning
												 * state */
...
typedef struct DynamicBitmapHeapScanState
{
	...
	/*
	 * lastRelOid is the last relation that corresponds to the varattno
	 * mapping of qual and target list. Each time we open a new partition, we
	 * will compare the last relation with current relation by using
	 * varattnos_map() and then convert the varattno to the new varattno
	 */
	Oid			lastRelOid;

	/*
	 * scanrelid is the RTE index for this scan node. It will be used to
	 * select varno whose varattno will be remapped, if necessary
	 */
	Index		scanrelid;
	...
	Bitmapset  *as_valid_subplans;	/* used to determine partitions during
									 * dynamic pruning */
	bool		did_pruning;	/* flag that is set once dynamic pruning is
								 * performed */
...
typedef struct FunctionScanState
{
	...
	bool		delayEagerFree; /* is is safe to free memory used by this
								 * node, when this node has outputted its last
								 * row? */
	...
	int			initplanId;		/* initplan is for function execute on
								 * initplan */
} FunctionScanState;
...
{
	ScanState	ss;				/* its first field is NodeTag */
	int			readptr;		/* index of work table's tuplestore read
								 * pointer */
...
typedef struct DynamicForeignScanState
{
	...
	bool		did_pruning;	/* flag that is set once dynamic pruning is
								 * performed */
	void	  **fdw_private_array;	/* array of fdw_privates for each
									 * partition's foreign scan */
...
typedef struct NestLoopState
{
	...
	bool		nl_innerSideScanned;	/* set to true once we've scanned all
										 * inner tuples the first time */
	bool		nl_qualResultForNull;	/* the value of the join condition
										 * when one of the sides contains a
										 * NULL */
} NestLoopState;
...
typedef struct HashJoinState
{
	...
	bool		reuse_hashtable;	/* Do we need to preserve hash table to
									 * support rescan */
} HashJoinState;
...
typedef struct SortState
{
	...
	SharedSortInfo *shared_info;	/* one entry per worker * Greenplum: per
									 * QE */

	bool		delayEagerFree; /* is it safe to free memory used by this
								 * node, when this node has outputted its last
								 * row? */
	TuplesortInstrumentation sortstats; /* holds stats, if the Sort is eagerly
										 * free'd */
...
#define FIELDNO_AGGSTATE_ALL_PERGROUPS 53
	...
	int			group_id;		/* GROUP_ID in current projection. This is
								 * passed to GroupingSetId expressions,
								 * similar to the 'grouped_cols' value. */
	int			gset_id;
...
typedef struct TupleSplitState
{
	...
	/*
	 * For each splitting tuple is mapping to a bitmap set depends on
	 * AggExprId, Only the input AttrNum in the bitmap set, other column set
	 * to null
	 */
	...
	/*
	 * For each splitting tuple is mapping to a bitmap set depends on vars of
	 * normal agg, Only the input AttrNum in the bitmap set, other column set
	 * to null.
	 */
...
typedef struct WindowAggState
{
	 ...
	 * variables. You lose on some optimizations in that case, so we use these
	 * flags to indicate if they don't contain any variables, to allow those
	 * optimizations in the usual case that they don't.
	 */
	bool		start_offset_var_free;
	bool		end_offset_var_free;

	bool		start_offset_valid; /* is startOffsetValue valid for current
									 * row? */
	bool		end_offset_valid;	/* is endOffsetValue valid for current
									 * row? */
...
typedef struct HashState
{
	...
	bool		hs_quit_if_hashkeys_null;	/* quit building hash table if
											 * hashkeys are all null */
	bool		hs_hashkeys_null;	/* found an instance wherein hashkeys are
									 * all null */
	/* hashkeys is same as parent's hj_InnerHashKeys */

	SharedHashInfo *shared_info;	/* one entry per worker * Greenplum: per
									 * QE */
...
typedef struct SplitUpdateState
{
	...
	AttrNumber	input_segid_attno;	/* attribute number of "gp_segment_id" in
									 * subplan's target list */
	AttrNumber	output_segid_attno; /* attribute number of "gp_segment_id" in
									 * output target list */
...
typedef struct MotionState
{
	...
	/* For motion send */
	bool		sentEndOfStream;	/* set when end-of-stream has successfully
									 * been sent */
	List	   *hashExprs;		/* state struct used for evaluating the hash
								 * expressions */
	struct CdbHash *cdbhash;	/* hash api object */
	int			numHashSegments;	/* number of segments to use when
									 * calculating hash */

	/* For Motion recv */
	int			routeIdNext;	/* for a sorted motion node, the routeId to
								 * get next (same as the routeId last returned
								 * ) */
	bool		tupleheapReady; /* for a sorted motion node, false until we
								 * have a tuple from each source segindex */
	...
	int			numTuplesToAMS; /* Number of tuples from child that were sent
								 * to AMS */
	int			numTuplesFromAMS;	/* Number of tuples received from AMS */
	int			numTuplesToParent;	/* Number of tuples either from child or
									 * AMS that were sent to parent */

	struct timeval otherTime;	/* time accumulator used in sending motion
								 * node to keep track of time spent getting
								 * the next tuple (not sending). this could
								 * mean time spent in another motion node
								 * receiving. */

	struct timeval motionTime;	/* time accumulator for time spent in motion
								 * node.  For sending motion node it is just
								 * the amount of time actually sending the
								 * tuple thru the interconnect.  For receiving
								 * motion node, it is the time spent waiting
								 * and processing of the next incoming tuple. */

	Oid		   *outputFunArray; /* output functions for each column (debug
								 * only) */
