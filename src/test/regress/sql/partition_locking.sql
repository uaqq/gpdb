...
-- Test locking behaviour for SELECT from root partition. The locks are acqired
-- when gp_keep_partition_children_locks is on from both planner and ORCA. In
-- case of ORCA the locks are taken only on statically selected leafs or on
-- all leafs otherwise similar to planner.
